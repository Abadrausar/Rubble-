...
func CommandIni_Create(state *nca4.State, params []*nca4.Value) {
	filehandle := len(iniFiles)

	iniFiles = append(iniFiles, ini.NewFile())

	state.RetVal = nca4.NewValueFromI64(int64(filehandle))
}

// Parse an ini file.
// 	ini:parse filecontents
// Returns the ini file handle or an error message. May set the Error flag.
func CommandIni_Parse(state *nca4.State, params []*nca4.Value) {
	filehandle := len(iniFiles)
	if len(params) != 1 {
...
func CommandIni_Format(state *nca4.State, params []*nca4.Value) {
	if len(params) != 1 {
	...
	state.RetVal = nca4.NewValue(ini.Format(iniFiles[filehandle]))
}
...
func CommandIni_GetValue(state *nca4.State, params []*nca4.Value) {
	if len(params) != 3 {
	...
	state.RetVal = nca4.NewValue(val)
}
...
func CommandIni_SetValue(state *nca4.State, params []*nca4.Value) {
	if len(params) != 4 {
