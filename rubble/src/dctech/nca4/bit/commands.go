...
func CommandLShift(state *nca4.State, params []*nca4.Value) {
	...
	state.RetVal = nca4.NewValueFromI64(params[0].Int64() << opp2)
	return
}
...
// 	rshift a [count]
// Returns the param shifted "count" places
func CommandRShift(state *nca4.State, params []*nca4.Value) {
	if len(params) != 1 && len(params) != 2 {
...
// 	and a b
// Returns a & b
func CommandAnd(state *nca4.State, params []*nca4.Value) {
	if len(params) != 2 {
...
// 	or a b
// Returns a | b
func CommandOr(state *nca4.State, params []*nca4.Value) {
	if len(params) != 2 {
...
// 	xor a b
// Returns a ^ b
func CommandXor(state *nca4.State, params []*nca4.Value) {
	if len(params) != 2 {
...
// 	not a
// Returns a ^ -1
func CommandNot(state *nca4.State, params []*nca4.Value) {
	if len(params) != 1 {
		panic("Wrong number of params to not.")
	}

	state.RetVal = nca4.NewValueFromI64(params[0].Int64() ^ -1)
	return
}
