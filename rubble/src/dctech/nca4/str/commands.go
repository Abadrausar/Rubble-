...
// 	append a b [c...]
// Returns "0" or "-1"
func CommandAppend(state *nca4.State, params []*nca4.Value) {
	if len(params) <= 1 {
	...
	state.RetVal = nca4.NewValue(result)
	return
}
...
// 	strlen a
// Returns the length
func CommandStrLen(state *nca4.State, params []*nca4.Value) {
	if len(params) != 1 {
		panic("Wrong param count to strlen.")
	}

	state.RetVal = nca4.NewValueFromI64(int64(len(params[0].String())))
	return
}
...
func CommandStrChar(state *nca4.State, params []*nca4.Value) {
	if len(params) != 2 {
	...
	state.RetVal = nca4.NewValue(string(params[0].String()[pos]))
	return
}
...
func CommandFmtStr(state *nca4.State, params []*nca4.Value) {
	if len(params) < 2 {
	...
	state.RetVal = nca4.NewValue(string(output))
	return
}
