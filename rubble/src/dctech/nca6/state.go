		...
		if x := recover(); x != nil {
			...
			case string:
				if this.Code.Last().Line() != -1 {
					err = fmt.Errorf("%v On Line: %v.", i, this.Code.Last().Line())
				} else {
					err = fmt.Errorf("%v", i)
				}
			default:
...
func (this *State) execCommand() {
	...
	case ncalex.TknString:
		...
		name = TokenToValue(this.Code.Last().CurrentTkn())
	...
	default:
		ncalex.ExitOnTokenExpected(this.Code.Last().LookAhead(), ncalex.TknString, ncalex.TknOpenParen, ncalex.TknOpenSqBracket)
	}
	...
	for !this.Code.Last().CheckLookAhead(ncalex.TknCloseParen) {
		...
		case ncalex.TknString:
			this.Code.Last().GetToken(ncalex.TknString)
			params = append(params, TokenToValue(this.Code.Last().CurrentTkn()))
		case ncalex.TknOpenParen:
			this.execCommand()
			if this.Return {
				return
		...
		case ncalex.TknOpenSqBracket:
			...
			params = append(params, this.RetVal)
		default:
			ncalex.ExitOnTokenExpected(this.Code.Last().LookAhead(), ncalex.TknString, ncalex.TknOpenParen, ncalex.TknOpenSqBracket, ncalex.TknCloseParen)
		}
	}
...
func (this *State) derefVar() {
	...
	index := NewValueString("")
	switch this.Code.Last().LookAhead().Type {
	case ncalex.TknString:
		this.Code.Last().GetToken(ncalex.TknString)
		index = TokenToValue(this.Code.Last().CurrentTkn())
	case ncalex.TknOpenParen:
		this.execCommand()
		index = this.RetVal
		if this.Return {
	...
	if name.Type != TypObject {
		object := this.GetValue(name.String())
		
		if object.Type != TypObject {
			panic("Non-Object value passed to an indexing deref opperator.")
		}
		if !object.IsIndexable() {
			panic("Non-Indexable object passed to an indexing deref opperator.")
		}
		if index.Type == TypObject {
			panic("Indexing deref opperator may not take an object as an index.")
		}
		
		val := object.Data.(Indexable)
		this.RetVal = val.Get(index.String())
		return
	}
