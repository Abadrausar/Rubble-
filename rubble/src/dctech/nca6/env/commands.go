// NCA v6 OS Environment Commands.
package env
...
// 	env:exec path [args]
// Returns unchanged.
func CommandEnv_Exec(state *nca6.State, params []*nca6.Value) {
	if len(params) < 1 {
...
func CommandEnv_GetVar(state *nca6.State, params []*nca6.Value) {
	if len(params) != 1 {
		panic("Wrong number of params to getenv.")
	}

	state.RetVal = nca6.NewValueString(os.Getenv(params[0].String()))
}

// Sets the value of an environment variable.
// 	env:setvar name value
// Returns the old value.
func CommandEnv_SetVar(state *nca6.State, params []*nca6.Value) {
	if len(params) != 2 {
		panic("Wrong number of params to setenv.")
	}
	rtn := nca6.NewValueString(os.Getenv(params[0].String()))
...
func CommandEnv_GetWD(state *nca6.State, params []*nca6.Value) {
	wd, err := os.Getwd()
	if err != nil {
		panic(err)
	}

	state.RetVal = nca6.NewValueString(wd)
}

// Set the working dir.
// 	env:setwd path
// Returns "".
func CommandEnv_SetWD(state *nca6.State, params []*nca6.Value) {
	if len(params) != 1 {
