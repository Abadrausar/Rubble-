...
func CommandCsv_Parse(state *nca5.State, params []*nca5.Value) {
	handle := len(csvFiles)
	if len(params) != 1 {
...
func CommandCsv_GetField(state *nca5.State, params []*nca5.Value) {
	if len(params) != 3 {
	...
	if record >= len(csvFiles[handle]) {
		state.Error = true
		state.RetVal = nca5.NewValue("error:Invalid Record.")
		return
	}
	...
	if field >= len(csvFiles[handle][record]) {
		state.Error = true
		state.RetVal = nca5.NewValue("error:Invalid Field.")
		return
	}

	state.RetVal = nca5.NewValue(csvFiles[handle][record][field])
}
...
func CommandCsv_RecordCount(state *nca5.State, params []*nca5.Value) {
	if len(params) != 1 {
	...
	state.RetVal = nca5.NewValueFromI64(int64(len(csvFiles[handle])))
}

// Gets a specific field from a csv file.
// 	csv:fieldcount handle record
// Returns the count or an error message. May set the Error flag.
func CommandCsv_FieldCount(state *nca5.State, params []*nca5.Value) {
	if len(params) != 2 {
	...
	state.RetVal = nca5.NewValueFromI64(int64(len(csvFiles[handle][record])))
}
