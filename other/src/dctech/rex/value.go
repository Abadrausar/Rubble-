...
func tokenToValue(tok *token) *Value {
	...
	// Variadic
	if tok.Type == tknVariadic {
		val.Type = TypVariadic
		val.Data = "..."
		return val
	}
...
// The value generated by this function is only good for display to a user.
...
func (val *Value) call(script *Script, names []string, params []*Value) {
	...
	if val.Type == TypCommand {
		// Native commands still do not have named params.
	...
	script.setParams(block, names, params)
