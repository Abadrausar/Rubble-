...
// NCA v7 Regular Expression Commands.
package regex
...
func CommandRegEx_Replace(state *nca7.State, params []*nca7.Value) {
	if len(params) != 3 {
		panic("Wrong number of params to regex:replace.")
	}

	regEx := regexp.MustCompile(params[0].String())
	state.RetVal = nca7.NewValueString(regEx.ReplaceAllString(params[1].String(), params[2].String()))
}
