...
func SetupBuiltins() {
	// This should be all the "fixed global templates" from Blast
	NewNativeTemplate("!TEMPLATE", tempTemplate)
	NewNativeTemplate("!T", tempTemplate)
	NewNativeTemplate("!GLOBAL_TEMPLATE", tempTemplate) // Not applicable to GoBlast, alias for !TEMPLATE
	...
	// AQUIFER was easier to impliment in Blast code
	tempTemplate([]string{"#AQUIFER", tempAquifer})
	
	NewNativeTemplate("#ADV_TIME", tempAdvTime)
	NewNativeTemplate("#FORT_TIME", tempFortTime) // I have no idea if this one is correct
	...
	// The next two are non-Blast, remember to suggest these
	...
	// Non-Blast
	NewNativeTemplate("CONFIG", tempConfig)
}
...
var tempAquifer = "{IFVAR;base__CONFIG_AQUIFER;YES;[AQUIFER];}"
...
func tempConfig(params []string) string {
	if len(params) != 2 {
		panic("Wrong number of params to CONFIG.")
	}
	
	name := CurFile.Namespace + "__CONFIG_" + params[0]
	varData[name] = params[1]
	return params[1]
}
